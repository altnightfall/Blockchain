...
class Address:
    ...
    def __init__(
        ...
        pkey: [None, PublicKey] = None,
        pkey_compressed: [None, str] = None,
    ) -> None:
        if pkey is not None:
...
class Transaction:
    ...
    def __init__(
        ...
        self.data = dict.fromkeys(
            ["ttype", "timestamp", "fromAddr", "toAddr", "pkey", "value", "fee", "msg"]
        )
        ...
        if datastring and datastring != self._datastring:
            raise Exception(
                "Datastrings from created and imported transactions don't match"
            )
    ...
    def validate(cls, datastring: str, signature: str) -> bool:
        ...
        data = cls.decodeDatastring(datastring)
        if (
            data["ttype"] != TTypes.creationReward and data["ttype"] != TTypes.fee
        ) and data["fromAddr"] is None:
            ret = False
            raise ValueError("Transaction don't have a fromAddress")
        if not Ecdsa.verify(
            datastring,
            Signature.fromBase64(signature),
            PublicKey.fromCompressed(data["pkey"]),
        ):
            ret = False
            raise ValueError("Transaction didn't pass signature validation")
        if (
            data["ttype"] != TTypes.creationReward and data["ttype"] != TTypes.fee
        ) and not (data["fromAddr"].getAddr()[-40:] == data["pkey"][-40:]):
            ret = False
            raise ValueError("Transaction sender don't own public key")
        if (
            data["ttype"] == TTypes.creationReward or data["ttype"] == TTypes.fee
        ) and not (data["fromAddr"] is None):
            ret = False
            raise ValueError(
                "Transaction can't have fromAddr if it's of type creationReward or fee"
            )
        if data["ttype"] == TTypes.creationReward and (
            data["value"] != Constants.CreationReward()
        ):
            ret = False
        ...
        if data["ttype"] == TTypes.creationReward and data["fee"] != 0:
            ret = False
            raise ValueError(
                "Fee must be 0 for transactions with a type creationReward"
            )
