...
class TransactionList:
    ...
    @classmethod
    def create(cls, ckey : PrivateKey, address : [None, Address] = None, *transactions : Transaction):
        pkey = ckey.publicKey()
        addr = Address(address, pkey if address is None else None)
        trcreation = Transaction(TTypes.creationReward, None, addr, pkey, Constants.CreationReward(), 0, ckey)
...
class Block:
    ...
    def __init__(self, id : int, prevHash : str, transactionList : TransactionList, nonce : int = 0, datastring : [None, str] = None, hash : [None, str] = None) -> None:
    ...
    @classmethod
    def validate(cls, block) -> bool:
        ret = True
        datastring = cls.encodeDatastring(block.data)
        if(not isinstance(block, Block)):
            ret = False
            raise ValueError("Is not a block")
        if(block.datastring != datastring):
            ret = False
            raise ValueError("Datastring is not right")
        hash = hashlib.sha256(block.datastring.encode('utf-8')).hexdigest()
        if(block.hash != hash):
            ret = False
        ...
        if(not isinstance(block.data['transactionList'], TransactionList) and not (block.data['transactionList'].validate())):
            ret = False
    ...
    def createInit(cls, ckey : PrivateKey):
        ...
        initTransaction = Transaction(TTypes.creationReward, None, addr, pkey, Constants.CreationReward(), 0, ckey)
        return cls(0, "FunnyMonke", TransactionList(initTransaction))
    
    @classmethod 
    def construct(cls, id : int, prevHash : str, ckey : PrivateKey, *transactions : Transaction):
        trlist = TransactionList.create(ckey, None, *transactions)
        block = cls(id, prevHash, trlist)
        block.mine()
        ret = cls.validate(block)
        if(ret):
            return block
        else:
