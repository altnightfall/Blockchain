...
class TestBlock:
    ...
    async def test_CreateBlock(self):
        ...
        assert (
            block.getTransaction(0).data["toAddr"].getAddr()
            == Address(pkey=self.pkeyMiner).getAddr()
        )
        assert (
            block.getTransaction(2).data["toAddr"].getAddr()
            == Address(pkey=self.pkeyMiner).getAddr()
        )
